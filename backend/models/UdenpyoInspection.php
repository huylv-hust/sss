...
class UdenpyoInspection
{
    ...
    public function getDataDiscount($parent_discount_id)
    {
        ...
        foreach ($packages as $k => $v) {
            $discount = $this->discount->getData(['DISCOUNT_PACKAGES_ID' => $v['ID']], 'ID,VALUE,DESCRIPTION');
            ...
            foreach ($discount as $discount_key => $discount_value) {
                $result[$v['ID']]['discount'][$discount_value['ID']]['VALUE'] = $discount_value['VALUE'];
                $result[$v['ID']]['discount'][$discount_value['ID']]['DESCRIPTION'] = $discount_value['DESCRIPTION'];
            }
        }
        return $result;
    }
